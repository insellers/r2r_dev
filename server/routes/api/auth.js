...
router.post("/register", (req, res) => {
  ...
  User.findOne({ email }, async (err, existingUser) => {
    ...
    const merchantMake = new Merchant({
    ...
    const merchantDoc = await merchantMake.save();
    const merchant = merchantDoc._id;
    const user = new User({
      email,
      password,
      firstName,
      ...
      bcrypt.hash(user.password, salt, (err, hash) => {
        if (err) {
          return res.status(400).json({
            error:
              "Your request could not be processed. Please try again. SALT",
          });
        }
        ...
        user.save(async (err, user) => {
          if (err) {
            return res.status(400).json({
              error:
                "Your request could not be processed. Please try again.SAVE" +
                err,
            });
          }
